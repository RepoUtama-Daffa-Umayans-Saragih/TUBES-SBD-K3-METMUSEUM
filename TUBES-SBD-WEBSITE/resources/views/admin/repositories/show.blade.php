    ...
    <div class="admin-section">
        ...
        <div class="admin-detail-header">
            ...
            <div class="detail-actions">
                <a href="{{ route('admin.repositories.edit', $repository) }}" class="btn btn-primary">Edit</a>
                <a href="{{ route('admin.repositories.index') }}" class="btn btn-secondary">Back</a>
            </div>
        </div>

        <div class="admin-detail-container">
            <div class="detail-card">
                <h3 class="detail-card-title">Repository Information</h3>
                <div class="detail-content">
                    <div class="detail-row">
                        <div class="detail-label">Repository Name</div>
                        <div class="detail-value">{{ $repository->repository_name }}</div>
                    </div>
                    <div class="detail-row">
                        <div class="detail-label">Total Artworks</div>
                        <div class="detail-value">{{ $repository->artWorks->count() }}</div>
                    </div>
                </div>
            </div>
            ...
            <div class="detail-card">
                ...
                <div class="detail-content">
                    @if ($repository->artWorks->count())
                        <table class="admin-table">
                            <thead>
                                <tr><th>Title</th><th>Accession</th><th>Actions</th></tr>
                            </thead>
                            <tbody>
                                @foreach ($repository->artWorks as $artwork)
                                    <tr>
                                        <td>{{ $artwork->title }}</td>
