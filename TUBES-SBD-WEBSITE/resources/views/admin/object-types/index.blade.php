...
@section('content')
    <div class="admin-section">
        ...
        <div class="admin-header">
            ...
            <a href="{{ route('admin.object-types.create') }}" class="btn-primary">+ Create Object Type</a>
        </div>
        ...
        <div class="admin-table-container">
            @if ($objectTypes->count())
                <table class="admin-table">
                    ...
                    <tbody>
                        @foreach ($objectTypes as $type)
                            <tr>
                                <td><strong>{{ $type->object_type_name }}</strong></td>
                                <td><span class="badge">{{ $type->art_works_count }}</span></td>
                                <td class="admin-actions">
                                    <a href="{{ route('admin.object-types.show', $type) }}" class="btn-sm btn-info">View</a>
                                    <a href="{{ route('admin.object-types.edit', $type) }}" class="btn-sm btn-warning">Edit</a>
                                    <form action="{{ route('admin.object-types.destroy', $type) }}" method="POST" style="display:inline;">
                                        @csrf @method('DELETE')
                ...
                <div class="admin-pagination">{{ $objectTypes->links() }}</div>
            @else
                <div class="admin-empty-state">
                    <p>No object types found</p>
                    <a href="{{ route('admin.object-types.create') }}" class="btn-primary">Create First</a>
                </div>
            @endif
        </div>
    </div>
