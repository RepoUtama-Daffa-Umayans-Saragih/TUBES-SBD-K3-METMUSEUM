    ...
    <div class="admin-section">
        ...
        <div class="admin-detail-header">
            ...
            <div class="detail-actions">
                <a href="{{ route('admin.mediums.edit', $medium) }}" class="btn btn-primary">Edit</a>
                <a href="{{ route('admin.mediums.index') }}" class="btn btn-secondary">Back</a>
            </div>
        </div>

        <div class="admin-detail-container">
            <div class="detail-card">
                <h3 class="detail-card-title">Medium Information</h3>
                <div class="detail-content">
                    <div class="detail-row">
                        <div class="detail-label">Medium Name</div>
                        <div class="detail-value">{{ $medium->medium_name }}</div>
                    </div>
                    <div class="detail-row">
                        <div class="detail-label">Total Artworks</div>
                        <div class="detail-value">{{ $medium->artWorks->count() }}</div>
                    </div>
                </div>
            </div>
            ...
            <div class="detail-card">
                ...
                <div class="detail-content">
                    @if ($medium->artWorks->count())
                        <table class="admin-table">
                            <thead>
                                <tr><th>Title</th><th>Accession</th><th>Actions</th></tr>
                            </thead>
                            <tbody>
                                @foreach ($medium->artWorks as $artwork)
                                    <tr>
                                        <td>{{ $artwork->title }}</td>
