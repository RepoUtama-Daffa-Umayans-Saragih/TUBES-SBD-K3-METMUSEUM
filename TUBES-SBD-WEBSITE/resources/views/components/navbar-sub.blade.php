<!-- HEADER -->
  <header class="w-full h-[92px] bg-[#e4002b] flex items-center justify-between px-10">
    ...
    <div class="flex items-center gap-10 text-white">
      ...
      @elseif(session()->has('guest_user') || session()->has('guest_id'))
      <div class="flex items-center gap-3">
        <span class="rounded-full border border-white/40 px-3 py-1 text-sm font-semibold uppercase tracking-[0.08em] text-white/90">
          Guest Mode
        </span>
        <form action="{{ route('account.logout') }}" method="POST" class="inline m-0 p-0">
          @csrf
