...
===========================
Scrapes primary image + gallery thumbnail images from the Metropolitan
Museum of Art artwork pages for the curated 2000 CSV dataset, then writes
the results as a FLAT JSON array to:

    database/data/image_curated_2000.json

One JSON object = one image row (DB-ready).

Features
--------
- Selenium + Chrome (handles JS-hydrated pages)
- Autosave every AUTOSAVE_EVERY records
- Resume mode: skips met_object_ids already represented in the output file
- Retry with back-off per page
...
Usage
-----
    python scrape_image_curated_2000.py
    python scrape_image_curated_2000.py --limit=50
    python scrape_image_curated_2000.py --headless=false
"""
...
OUTPUT_JSON = "database/data/image_curated_2000.json"
...
def detect_blocked(page_source: str) -> bool:
    ...
    markers = ["security checkpoint", "verify your browser", "captcha", "access denied", "unusual traffic", "cloudflare"]
...
def detect_restricted(driver, met_object_id: int) -> bool:
    """Heuristic: artwork is rights-restricted if no IIIF URL is found or if the page shows a restricted notice."""
    try:
...
def main() -> None:
    print("=" * 60)
    print("  scrape_image_curated_2000.py  –  Met Museum Image Scraper")
    print("=" * 60)
