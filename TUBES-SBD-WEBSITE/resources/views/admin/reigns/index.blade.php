...
@section('content')
    <div class="admin-section">
        ...
        <div class="admin-header">
            ...
            <a href="{{ route('admin.reigns.create') }}" class="btn-primary">+ Create Reign</a>
        </div>
        ...
        <div class="admin-table-container">
            @if ($reigns->count())
                <table class="admin-table">
                    ...
                    <tbody>
                        @foreach ($reigns as $reign)
                            <tr>
                                <td><strong>{{ $reign->reign_name }}</strong></td>
                                <td><span class="badge">{{ $reign->art_works_count }}</span></td>
                                <td class="admin-actions">
                                    <a href="{{ route('admin.reigns.show', $reign) }}" class="btn-sm btn-info">View</a>
                                    <a href="{{ route('admin.reigns.edit', $reign) }}" class="btn-sm btn-warning">Edit</a>
                                    <form action="{{ route('admin.reigns.destroy', $reign) }}" method="POST" style="display:inline;">
                                        @csrf @method('DELETE')
                ...
                <div class="admin-pagination">{{ $reigns->links() }}</div>
            @else
                <div class="admin-empty-state">
                    <p>No reigns found</p>
                    <a href="{{ route('admin.reigns.create') }}" class="btn-primary">Create First</a>
                </div>
            @endif
        </div>
    </div>
